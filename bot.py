...
class Database:
    ...
    def _default_batch(self, name: str) -> dict:
        return {
            ...
            "bottom_post": None,
            "custom_link": None,   # str URL — overrides the bot-generated Sub Channel invite link
                                    # on the Main Channel(s) 🍁DOWNLOAD🍁 button when set
...
def kb_batch_edit_menu(batch_id: str) -> InlineKeyboardMarkup:
    rows = [
        ...
        [InlineKeyboardButton("🔗 Set Custom Link", callback_data=f"f_customlink_{batch_id}")],
...
async def process_job(job: dict):
    ...
    if main_channels:
        # A batch-level Custom Link (set via /edit_batch → 🔗 Set Custom Link) always wins over
        # the bot's own auto-generated Sub Channel invite link. Each batch's custom link is
        # independent, so different batches can point at completely different destinations.
        custom_link = batch.get("custom_link")
        join_link = custom_link if custom_link else await get_channel_join_link(app, sub_channel["id"])
        button = None
        if join_link:
...
async def process_torrent_job(job: dict):
    ...
    try:
        ...
        if main_channels and sub_channel and sub_channel.get("id"):
            # Same rule as the normal file flow: a batch-level Custom Link wins over the
            # bot-generated Sub Channel invite link, per-batch.
            custom_link = batch.get("custom_link")
            join_link = custom_link if custom_link else await get_channel_join_link(app, sub_channel["id"])
            button = None
            if join_link:
...
async def conversation_input_handler(client, message: Message):
    ...
    if action == "edit_field":
        ...
        if field == "customlink":
            if not message.text:
                await message.reply_text("❌ Please send this as text (a link), or /cancel.")
                return
            text = message.text.strip()
            if text.lower() in ("clear", "remove", "none", "-"):
                await db.update_batch_field(batch_id, "custom_link", None)
                conversation_state.pop(uid, None)
                await message.reply_text(
                    "🗑️ **Custom link removed.** The bot-generated Sub Channel invite link will be used on the "
            ...
            if not re.match(r'^https?://\S+$', text):
                await message.reply_text(
                    "❌ That doesn't look like a link. Send a full URL starting with `http://` or `https://`, "
                    "send `clear` to remove an existing one, or /cancel."
                )
                return
            await db.update_batch_field(batch_id, "custom_link", text)
            conversation_state.pop(uid, None)
            await message.reply_text(
                f"✅ **Custom link set:**\n{text}\n\n"
                "This will now be used on the 🍁DOWNLOAD🍁 button under this batch's Main Channel post, instead "
                "of the bot-generated Sub Channel invite link.",
                reply_markup=kb_back_field(batch_id),
            )
            return
...
FIELD_PROMPTS = {
    ...
    "coverimage": "🎨 **Cover Image**\n\nSend a **photo** to use as this batch's full-quality video poster "
                  ...
                  "chat preview).",
    "customlink": "🔗 **Custom Redirect Link**\n\nSend the full URL you want used on the 🍁DOWNLOAD🍁 button "
                  "under this batch's Main Channel post, instead of the bot-generated Sub Channel invite link.\n\n"
                  "Must start with `http://` or `https://`.\n\n"
                  "Send `clear` to remove it and go back to using the bot-generated invite link.",
}
...
async def show_batch_info(cq, batch_id: str):
    ...
    lines = [
        ...
        channel_line,
        main_line,
        f"🔗 Custom Link: {'✅ ' + clip_for_display(batch.get('custom_link'), 200) if batch.get('custom_link') else '❌ Not set (using bot-generated invite link)'}",
    ]
...
async def callback_handler(client, cq):
    ...
    try:
        ...
        if data.startswith("f_"):
            ...
            elif field in ("autorename", "autocaption", "toppost", "bottompost", "customlink"):
                conversation_state[uid] = {"action": "edit_field", "field": field, "batch_id": batch_id}
                await cq.message.edit_text(FIELD_PROMPTS[field] + "\n\nUse /cancel to abort.", reply_markup=kb_back_field(batch_id))
